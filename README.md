...
This configuration includes support for using a Seeed Studio XIAO nRF52840 as a wireless USB dongle. The dongle acts as a BLE central device that receives input from both keyboard halves and forwards it to your computer via USB.

## Benefits of Using a Dongle

* **Single USB Connection**: Connect only the dongle to your computer, not the keyboard halves
* **Battery-Powered Keyboard**: Both keyboard halves run on batteries without USB cables
* **Reduced Latency**: Direct BLE connection to a dedicated receiver can reduce overall latency
* **Cleaner Desk Setup**: No cables running to the keyboard

## Hardware Required

* **Seeed Studio XIAO nRF52840** board (for the dongle)
* Two **nice!nano v2** boards (for left and right keyboard halves)
...
* `leeloo_dongle-xiao_ble-zmk.uf2` - Dongle firmware
* `leeloo_rev2_left-nice_nano_v2-*.uf2` - Left keyboard half
* `leeloo_rev2_right-nice_nano_v2-*.uf2` - Right keyboard half

To build the dongle firmware locally:
```
west build -d build/dongle -p -b xiao_ble -- -DSHIELD=leeloo_dongle -DZMK_CONFIG="path/to/config"
```

## Flashing and Setup

1. **Flash the dongle**: Put the XIAO nRF52840 into bootloader mode (double-tap reset button) and copy the `leeloo_dongle-xiao_ble-zmk.uf2` file to the drive that appears
2. **Flash both keyboard halves**: Flash the standard left and right firmware files to your nice!nano boards
3. **Power on**: Connect the dongle to your computer via USB and power on both keyboard halves
4. **Automatic pairing**: The keyboard halves will automatically pair with the dongle when powered on

**Note**: The dongle must be connected to use the keyboard. The keyboard halves will not connect directly to your computer when using the dongle setup.
